...
app.get('/api/youtube-subcount/:channelId', async (req, res) => {
  ...
  try {
    ...
    const response = await axios.get(`https://livecounts.xyz/api/youtube-live-subscriber-count/live/${channelId}`);
    const respons2e = await axios.get(`https://backend.mixerno.space/api/youtube/estv3/${channelId}`);
    ...
    res.json({ stats: {subCount, totalViews, apiSubCount, videos, apiViews}, info: { channelLogo, channelName, channelBanner}});
  } catch (error) {
    console.error(error);
...
app.get('/api/youtube-subcount/studio/:channelId', async (req, res) => {
  ...
  try {
    // Fetch data from the external API
    const response = await fetch(`https://cors.stats100.xyz/https://studio.nia-statistics.com/api/channel/${channelId}`);
    const respons2e = await axios.get(`https://backend.mixerno.space/api/youtube/estv3/${channelId}`);
...
app.get('/api/mrbeast/youtube-subcount/', async (req, res) => {
  ...
  try {
    ...
    const respons2e = await axios.get(`https://backend.mixerno.space/api/youtube/estv3/UCX6OQ3DkcsbYNE6H8uQQuVA`);
    ...
    res.json({ stats: {subCount, viewCount, apiSubCount, videos, apiViews, vsCount, vsUpdateTime}, item: { channelLogo, channelName, channelBanner}});
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: 'Failed to fetch subscriber count' });
  }
